...
-	Input parameters are:  \
	...
	**clustering_dir:** Input directory containing the cluster prediction of patches in each WSI. For each slide, a file named ```<slide_name><*>.csv``` contains the following columns: ```coord_x```, ```coord_y```, ```cluster_id```  The clusters are predicted using the clustering centroids used in the model training.   
