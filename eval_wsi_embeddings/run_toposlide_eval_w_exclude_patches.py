...
'''
python run_toposlide_eval_w_exclude_patches.py  \
--patch_embedding_dir "/oak/stanford/groups/plevriti/shahira/datasets/tcga_luad/patches_20x_512_conch_tile_embedding" \
--patches_root_output_dir "/oak/stanford/groups/plevriti/shahira/datasets/tcga_luad/patches_20x_512_conch_toposlide_tile_embedding_e215" \
--checkpoint_path "/oak/stanford/groups/plevriti/shahira/code/TopoSlide/trained_models/toposlide_tcga_luad.pt" \
--clustering_dir "/oak/stanford/groups/plevriti/shahira/datasets/tcga_luad/patches_20x_512_conch_tile_embedding_kmeans_n16/prediction_kmeans" \
--wsi_dim_filepath "/oak/stanford/groups/plevriti/shahira/tcga_luad_wsi_dim_p512_m20.csv" \
--ignore_cluster_ids  \
--n_clusters 16 
'''
...
if __name__=="__main__":
    ...
    # parser.add_argument("--slide_root_output_dir", type=str, default="/oak/stanford/groups/plevriti/shahira/datasets/tcga_luad/patches_20x_512_toposlide_wsi_embedding")
    parser.add_argument("--patches_root_output_dir", type=str, default="/oak/stanford/groups/plevriti/shahira/datasets/tcga_luad/patches_20x_512_conch_toposlide_tile_embedding")
    ...
    patches_root_output_dir = args.patches_root_output_dir
    print('ignore_clusters_arr', ignore_clusters_arr)
    ...
    if(len(ignore_clusters_arr) > 0 and (min(ignore_clusters_arr)<0 or max(ignore_clusters_arr)>=n_clusters)):
    ...
    os.makedirs(patches_root_output_dir, exist_ok=True)
    ...
    config.vision_config.return_all_tokens_coord = True
    print('config done')
    ...
    for indx in np.random.permutation(wsi_dim_arr.shape[0]):        
        ...
        patches_out_filepath = os.path.join(patches_root_output_dir, os.path.basename(patch_feat_filepath))
        if(os.path.exists(patches_out_filepath)):
            continue
        print('slide_name', slide_name)
        ...
        #     ti += 1
        tile_size = pw
        scale = tile_cluster_size/tile_size
        ...
        for ti in range(len(tiles_coord_scaled_x)):
            if(tiles_cluster_id[ti] in ignore_clusters_arr):
                continue
            cluster_id_map[tiles_coord_scaled_y[ti]:min(new_height, tiles_coord_scaled_y[ti]+tile_cluster_size), tiles_coord_scaled_x[ti]:min(new_width, tiles_coord_scaled_x[ti]+tile_cluster_size)] = tiles_cluster_id[ti]
        ...
        with torch.autocast('cuda', torch.float16), torch.inference_mode():
            ...
            slide_embedding, tokens_logits, token_coord = model.encode_slide_from_patch_features(features, coords, patch_size_lv0)
            tokens_logits_strict = tokens_logits[:,1:]
            tokens_logits_cls = tokens_logits[:,1]

            with h5py.File(patches_out_filepath, 'w') as out_hf:
                out_hf.create_dataset('coords', data = np.asarray(token_coord.detach().squeeze().permute(1,0).cpu().numpy(), dtype=int), dtype=int, compression="gzip")
                out_hf.create_dataset('features', data = np.asarray(tokens_logits_strict.detach().squeeze().cpu().numpy(), dtype=np.float16), dtype=np.float16, compression="gzip")                
                out_hf['patch_size_lv0'] = patch_size_lv0
                out_hf.close()
