...
splits_folder = f"/oak/stanford/groups/plevriti/shahira/datasets/cptac_brca/data_splits/train_test_5fold_site_preserving_surv_patientwise"
...
if __name__=="__main__":
    ...
    parser.add_argument("--embedding_dir", type=str, default="/oak/stanford/groups/plevriti/shahira/datasets/cptac_brca/patches_20x_512_titan_slide_embedding")
    parser.add_argument("--out_root_dir", type=str, default="/oak/stanford/groups/plevriti/shahira/wsi_rep_eval_results/survival_reg/cptac_brca")
    parser.add_argument("--method_name", type=str, default="titan_original")
    parser.add_argument("--survival_filepath", type=str, default="/oak/stanford/groups/plevriti/CPTAC_WSI/METADATA/CPTAC_BRCA_survival.csv")
    parser.add_argument("--splits_folder", type=str, default="/oak/stanford/groups/plevriti/shahira/datasets/cptac_brca/data_splits/train_test_5fold_site_preserving_surv_patientwise")
    parser.add_argument("--wsi_dim_filepath", type=str, default="/oak/stanford/groups/plevriti/shahira/cptac_brca_wsi_dim.csv")
    ...
    for slide_filepath in slide_embedding_files:
        ...
        slide_ids_main.append(slide_name.split('-')[0])
        # print(slide_name)
        print(wi, slide_name.split('-')[0], end='\r')
    ...
    survival_df['PFI.time_months'] = survival_df['PFS_days']/30
    # survival_df = survival_df[pd.notnull(survival_df["PFI.time"])] # exclude nan
    survival_df = survival_df[pd.notnull(survival_df["PFI.time_months"])] # exclude nan
    survival_df = survival_df[pd.notnull(survival_df["PFS_event"])] # exclude nan
    surv_case_ids = survival_df['case_id'].to_numpy()
    # survival_slide_ids = survival_df['case_id'].to_numpy()
    # surv_pfi = survival_df['PFS_days'].values
    # surv_censored = survival_df['PFS_event'].values
    ...
    # slides_missing_he = []
    # slides_missing_surv = []
    include_slide = np.zeros(len(slide_ids) ,dtype=bool)
    for idx, slide_name in enumerate(slide_ids):
        case_id = slide_name.split('-')[0]
        # print('slide_name', slide_name, 'case_id ', case_id )
        if(case_id in surv_case_ids):
            include_slide[idx] = True
            slide_surv_df = survival_df[survival_df['case_id']==case_id]
            survival_slide_ids_new.append(slide_name)
            surv_pfi.append(slide_surv_df['PFI.time_months'].values[0])
            surv_censored.append(slide_surv_df['PFS_event'].values[0])
            # survival_slide_ids_new
        # else:
        #     print('case_id Not in surv_case_ids')
    print('survival_slide_ids_new', len(survival_slide_ids_new))
    ...
    #             surv_censored.append(slide_surv_df['censored'].values[0])
    ...
    print('surv_pfi', len(surv_pfi))

    # surv_dss_y = list(zip(surv_censored, surv_dss))
    surv_dss_y = np.array(list(zip(surv_censored.astype(bool), surv_pfi)), dtype=[('Event_indicator', bool), ('Time', float)])
    ...
    with open(os.path.join(out_dir, f"{method_name}_cox_fit_b.csv"), "w") as out_file:
        ...
        surv_data_x = slide_embeddings_arr[include_slide]
        ...
        for alpha in alphas:
            ...
            for ki in range(n_splits):
                ...
                train_slide_ids_main = np.array([s.split('-')[0] for s in train_slide_names]).astype(str)   
                test_slide_ids_main = np.array([s.split('-')[0] for s in test_slide_names]).astype(str)   
                ...
                try:
                    ...
                    train_slides, train_indices0, train_indices1= np.intersect1d(survival_slide_ids_new, train_slide_names, return_indices=True)
                    test_slides, test_indices0, test_indices1= np.intersect1d(survival_slide_ids_new, test_slide_names, return_indices=True)
