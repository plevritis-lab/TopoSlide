'''
    Adapted from https://huggingface.co/MahmoodLab/TITAN/blob/main/vision_transformer.py
'''
import math
...
def preprocess_features(features: torch.Tensor, coords: torch.Tensor, patch_size_lv0: int):
    # print('patch_size_lv0', patch_size_lv0)
    ...
    feature_grid = torch.zeros((_H, _W, features.size(-1)), device=features.device, dtype=features.dtype) # shahira added dtype
    ...
    # Background mask
    # print('feature_grid != 0', (torch.any(feature_grid != 0, dim=0)).sum())
    # print('feature_grid', feature_grid.shape)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            #### shahira ####
            return_all_tokens_before_pooling=False, 
            return_all_tokens_coord=False,
        ...
        self.return_all_tokens_before_pooling = return_all_tokens_before_pooling # shahira
        self.return_all_tokens_coord = return_all_tokens_coord # shahira
    ...
    def forward_features(self, x, coords=None, mask=None, bg_mask=None):
        ...
        if self.pos_encode_type == 'alibi':
            ...
            attn_bias = None
            # print('attn_bias = None')
        
        if self.masked_im_modeling:
            # print('self.masked_im_modeling')
        ...
        # print('_pos_embed x', x.shape)
        x = self.norm_pre(x)
        # print('norm_pre x', x.shape)
        
        # mask background tokens when evaluating (batch size = 1)
        if bg_mask is not None and B == 1:
            # print('bg_mask is not None')
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            # print('checkpoint_seq x', x.shape)
        else:
    ...
    def forward_features_patch_embed(self, x):
        x = self.patch_embed(x)
        return x
    ...
    def forward(self, patch_features, patch_coords, patch_size_lv0, return_all_tokens=False, mask=None, bg_mask=None, no_proj=False):
        # print('bg_mask is None', bg_mask is None)
        ...
        if self.attn_pool is not None: ### CoCa
            ...
            if no_proj:
                if self.return_all_tokens and self.return_all_tokens_before_pooling and self.return_all_tokens_coord: # shahira
                    if(bg_mask is not None):
                        # return pooled, tokens, x, coords[bg_mask.unsqueeze(0).repeat(1,2,1,1)]
                        return pooled, tokens, x, coords[:,:,bg_mask.squeeze(0)]
                    else:
                        return pooled, tokens, x, coords
                if self.return_all_tokens and self.return_all_tokens_before_pooling: # shahira
                    return pooled, tokens, x
                if self.return_all_tokens and self.return_all_tokens_coord: # shahira
                    if(bg_mask is not None):
                        # return pooled, tokens, coords[bg_mask.unsqueeze(0).repeat(1,2,1,1)]
                        return pooled, tokens, coords[:,:,bg_mask.squeeze(0)]
                    else:
                        return pooled, tokens, coords                    
                if self.return_all_tokens: # shahira
                    return pooled, tokens
                if self.return_all_tokens_before_pooling and self.return_all_tokens_coord: # shahira
                    if(bg_mask is not None):
                        # return pooled, x, coords[bg_mask.unsqueeze(0).repeat(1,2,1,1)]
                        return pooled, x, coords[:,:,bg_mask.squeeze(0)]
                    else:
                        return pooled, x, coords
                if self.return_all_tokens_before_pooling: # shahira
                    return pooled, x
                return pooled
...
def build_vision_tower(
    ...
    vision = VisionTransformer(
        ...
        return_all_tokens=vision_cfg.return_all_tokens, # shahira
        return_all_tokens_before_pooling=vision_cfg.return_all_tokens_before_pooling, # shahira
        return_all_tokens_coord = vision_cfg.return_all_tokens_coord # shahira
    )
    return vision